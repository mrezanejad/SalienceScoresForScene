...
% -------------------------------------------------------------------------
function boundaryScoreMatrix = get_tangent_point_scores_min(skeleton_contours,imsize,sbXY,simpleBoundaryInds,skeltonImageWithRating)
...
for i = 1 : length(skeleton_contours)
    ...
    if(size(FP)>0)
        ...
        if(~isempty(T))
            ...
            T1 = find(currentScores == 0);
            T2 = find(newScores == 0);
            currentScores(T1) = newScores(T1);
            newScores(T2) = currentScores(T2);
            
            boundaryScoreMatrix(reconstructedInds) = 1-min(newScores,currentScores);
        end
    end
end
